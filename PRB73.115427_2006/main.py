...
class grooving_iso(object):
    ...
    def mesh_uniform(self):
        for i in range(0,self.mp):
            ...
            self.msh[i][1]=1.0e-14
        return

    def update_b_l_2D(self):
    ...
    def get_gm(self,i): # gamma surface energy
        if i == 0:
            gm1=0
            gm2=0.77*1e-4 # J/cm^2
        else:
            gm1=0.77*1e-4 # J/cm^2
            gm2=0.77*1e-4 # J/cm^2
        return gm1,gm2
    def get_mu_i(self,i):
        ...
        if i == 0 :
            ...
            b2=self.msh[i][3]
            l2=self.msh[i][5]
            mu_i =mu_u0 +  2.0/b2*((self.msh[i][1]*gm2/l2)-(b2*gm2p/l2)-(gm2*self.cos_contact))
        else:
            self.update_b_l_2D()
            b1=self.msh[i][2]
            b2=self.msh[i][3]
            ...
            gm1p =  0
            gm2p =  0
            mu_i = mu_u0 + 2.0/(b1+b2)*(self.msh[i][1]*(gm1/l1+gm2/l2)+(b2*gm2p/l2-b1*gm1p/l1))
        return mu_i
    ...
    def get_flux_pt_i(self,i):
        if i==0:
            ...
            flux_n=-(self.D*self.gamma*self.omega*self.omega/self.kbT)*grd_s_mu
            flux_x=flux_n
        elif i== (self.mp-1):
            flux_x=0
        elif i== (self.mp-2):
            ...
            grd_s_mu=self.get_grad_s_mu_i(i)[0]
            flux_p=-(self.D*self.gamma*self.omega*self.omega/self.kbT)*grd_s_mu
    ...
    def update_coor_all(self):
        for i in range(self.mp):
            dz=self.msh[i][8]/(self.msh[i][2]+self.msh[i][3])
            if i == 0:
                self.msh[i][1]+=dz
            elif i == (self.mp-1):
                self.msh[i][1]+=0
        ...
        if (i+1)%100 == 0:
            ...
            with open(filename,'w')as fin:
                ...
                for j in range(self.mp):
                    fin.write('%12.9f    %12.9f\n'%(self.msh[j][0]*1e7,self.msh[j][1]*1e7))
        return
    ...
    def __init__(self):
        ...
        self.D,self.gamma,self.kbT=4.3e-4,1.8*1e-4,1.38e-23*700.0
        ...
        self.cos_contact=0.14 
        self.dt=1.0e5  #unit second
        self.main_loop()
        return
